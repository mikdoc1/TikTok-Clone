...
const Aside = () => {
    ...
    useEffect(() => {
        ...
        db.collection('discover').doc('explore').get()
            .then((musics) => {
                musics.data().exploreInfos.slice(0, 3).forEach(music => {
        ...
        .then((tags) => {
            tags.data().exploreInfos.slice(0, 3).forEach(tag => {
                setDiscvoer(discover => ({
                    ...discover,
                    tags: [
                        ...discover.tags,
    ...
    return (
        <div className="aside__recommendation">
            ...
            <ul className="recommend-list">
                {users.map(user => {
                    return (
                        <RecommendListItem  link={`/profile/${user.username}`}
                                            src={user.avatar}
                                            title={user.name}
            ...
            <div className="recommend-card" >
                ...
                <ul className="recommend-list" >
                    {discover.music.map(item => {
                        return (
                            <RecommendListItem  link={`/music/${item.title.replace(/\s/g,"-")}`} 
                                                src={item.avatar}
                                                title={item.title}
                                                subtitle={item.subtitle}
                                                key={item.subtitle}>
                                <img width="15" height="15" src="https://upload.wikimedia.org/wikipedia/commons/thumb/f/f9/Antu_arrow-right.svg/32px-Antu_arrow-right.svg.png" alt="arrow"/>
                            </RecommendListItem>
                        )
                    })}
                    {discover.tags.map(tag => {
                        return (
                            <RecommendListItem  link={`/tag/${tag.title.substring(1)}`} 
                                                src={tag.avatar}
                                                title={tag.title}
                                                subtitle={tag.subtitle}
                                                key={tag.subtitle}>

...
import withAuthPhoneNumber from '../../../HOC/withAuthWithPhoneNumber';

const LoginWithPhoneOrEmail = (props) => {
    ...
    return (
        <> 
            ...
            <form className="login-form__body-top" onSubmit={(e) => { e.preventDefault(); props.onLoginWithPhoneNumber()} } style={{textAlign: 'left', width: '100%'}}>
                <div className="login-form__body-top-title" style={{margin: '1rem 0'}}>
                    <p className="title title_black_bold">Phone</p>
                    <button className="title-smallest login-form__body-top-login-text">Log in with email or username</button>
                </div>
            ...
            <button onClick={(e) => context.onLogin(e, props.response)} className={props.loginBtnClass.join(' ')} style={{width: '100%', marginTop: '3rem'}}>Login</button>
...
export default withAuthPhoneNumber(LoginWithPhoneOrEmail);